...
@app.route('/')
def home():
    return "Чернейший оракул без совы в сети!"
...
# Системный промпт (Урезали длину, убрали сову)
SYSTEM_PROMPT = (
    ...
    "Шути самый жесткий черный юмор про семью, родственников, болезни, дурку и тотальный абсурд. "
    "СТРОГОЕ ПРАВИЛО: Пиши все матерные слова ЦЕЛИКОМ БУКВАМИ. Никаких звездочек. "
    "ОТВЕЧАЙ КОРОТКО И ЕМКО. Максимум 2-4 предложения. Не лей воду, выдавай только концентрированный трэш. "
...
def get_ai_response(chat_id, user_text, special_mode=None, chosen_card=None, user_number=None):
    ...
    if special_mode == "quote":
        prompt_text = "Выдай короткую, максимально дикую, матерную и чернушную мысль про жизнь или родственников. В 2 предложения."
    elif special_mode == "joke":
        prompt_text = "Расскажи один ОЧЕНЬ КОРОТКИЙ и максимально черный анекдот с кучей мата про болезни или безумную семью."
    elif special_mode == "oracle_card":
        prompt_text = (
            f"Пользователь выбрал число {user_number}. Обыграй карту '{chosen_card}' из мемных гаданий ТикТока. "
            f"ОТВЕЧАЙ КОРОТКО (максимум 3-4 предложения). НЕ ПИШИ КАПСОМ. "
            f"Выдай жесткое, упоротое и матерное предсказание будущего по этой карте. "
            f"Используй черный юмор: приплети туда безумную семью пользователя, их болезни, дурку, маразм и трэш. "
            f"Никаких упоминаний сов! Никаких звездочек в тексте!"
        )
    else:
    ...
    for model in models_to_try:
        payload = {
            ...
            "max_tokens": 250  # Жестко ограничили ответ на уровне API, чтобы не разгонялся
        }
        try:
