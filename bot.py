...
SYSTEM_PROMPT = (
    ...
    "НЕ пиши текст КАПСОМ (большими буквами). Пиши обычным текстом. "
...
def get_ai_response(chat_id, user_text, special_mode=None, chosen_card=None, user_number=None):
    ...
    if special_mode == "quote":
        ...
        prompt_text = (
            ...
            f"СТРОГОЕ ПРАВИЛО: НЕ ПИШИ КАПСОМ. Отвечай обычными маленькими буквами. "
            f"Выдай полнейшее шизофреничное, упоротое и матерное предсказание будущего по этой карте. "
...
def process_card_prediction(message):
    ...
    # Отправляем сообщение про колоду и сохраняем его id, чтобы потом удалить
    temp_msg = bot.send_message(chat_id, f"Принял число {user_num}... Тасую колоду из говна и палок, сука...")
    bot.send_chat_action(chat_id, 'typing')
    ...
    # Удаляем сообщение «Принял число...», чтобы очистить чат
    try:
        bot.delete_message(chat_id, temp_msg.message_id)
    except Exception:
        pass
        
    # Формируем красивый ответ: название карты жирным, а дальше — текст обычными буквами
